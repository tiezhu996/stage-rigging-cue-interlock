...
func (s *RehearsalRunService) Compare(id, otherID uint) (map[string]any, error) {
	...
	return map[string]any{
		...
		"delta":    map[string]any{"result_count": len(left.RuleResults) - len(right.RuleResults), "collision_windows": len(left.CollisionWindows) - len(right.CollisionWindows), "same_snapshot": left.ID == right.ID},
...
func cueSetVersion(cues []interlock.CueInput, rules []interlock.RuleInput, devices []model.RiggingDevice) string {
	...
	parts := make([]string, 0, len(cues)+len(rules))
