...
func (s *RiggingDeviceService) withRules(item model.RiggingDevice) (dto.RiggingDeviceResponse, error) {
	...
	var matched map[uint]dto.RuleReference
	for _, rule := range rules {
		...
		for _, ruleDeviceID := range ids {
			if ruleDeviceID == item.ID {
				matched[rule.ID] = dto.RuleReference{ID: rule.ID, RuleCode: rule.RuleCode, RuleType: rule.RuleType, Severity: rule.Severity, Enabled: rule.Enabled, RuleVersion: rule.RuleVersion}
			}
		}
	}
	for _, reference := range matched {
		response.ApplicableRules = append(response.ApplicableRules, reference)
	}
	return response, nil
}
