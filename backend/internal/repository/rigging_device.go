...
func (r *RiggingDeviceRepository) Create(item *model.RiggingDevice, event audit.Event) (err error) {
	tx := r.db.Begin()
...
func (r *RiggingDeviceRepository) Update(item *model.RiggingDevice, expectedVersion uint, event audit.Event) (err error) {
	tx := r.db.Begin()
	defer func() { err = tx.Commit().Error }()
