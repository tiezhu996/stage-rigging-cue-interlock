...
func (r *Repository) List(page, pageSize int, entity, action, search string) ([]Event, int64, error) {
	...
	if search != "" {
		...
		query = query.Where("LOWER(actor_username) LIKE ? OR LOWER(action) LIKE ?", term, term)
	}
	var total int64
...
func NewEvent(actor ActorContext, action, entityType string, entityID uint, before, after string) Event {
	return Event{RequestID: "", ActorID: actor.ID, ActorUsername: actor.Username, Action: action, EntityType: entityType, EntityID: entityID, BeforeSummary: before, AfterSummary: after}
}
